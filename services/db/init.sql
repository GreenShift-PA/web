...
-- Creation of the LikedBy database 
CREATE TABLE `green-shift`.`LikedBy` 
    (`ID` INT NOT NULL AUTO_INCREMENT , 
    `user_id` INT NOT NULL , 
    `post_id` INT NOT NULL , 
    PRIMARY KEY (`ID`)) ENGINE = InnoDB;
ALTER TABLE `LikedBy` ADD CONSTRAINT `like_post_id` FOREIGN KEY (`post_id`) REFERENCES `Post`(`ID`) ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE `LikedBy` ADD CONSTRAINT `like_user_id` FOREIGN KEY (`user_id`) REFERENCES `User`(`ID`) ON DELETE RESTRICT ON UPDATE RESTRICT;